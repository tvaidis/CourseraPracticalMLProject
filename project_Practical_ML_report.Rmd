...
Although not critical for the final model analysis, correlation patterns among predictors are 
explored graphically via the corrplot package, as shown in Fig.1 below:  


```{r corrplot,fig.height=8,fig.width=8}
#-- Check correlation patterns in features --
corrplot(corr=cor(trainRaw.df[,2:53]),method="circle",diag=F,type="lower",tl.cex=0.8)
```
  
__Fig.1: Correlation patterns among predictors__    
...
## Explaining power of predictors  
...
```{r varImportance,fig.height=8,fig.width=7}
...
__Fig.2: Predictor Importance for random forest model__  
...
xt <- xtable(model.rf$finalModel$confusion,digits=c(0,0,0,0,0,0,-2))
print(xt, type="html")
```  

__Table 1: In sample error table for all outcome classes__


### Expected out of sample error

To estimate the out of sample error  the values of the "classe" variable were predicted for the test dataset (30% portion of original training set).   
The model provided an estimated out of sample error of **0.32%**
...
```
__Expected Out of sample error for random forest model: `r  model.rf_OOS_error*100`%__
